...
Scenario:
1. Generate two branches. Main branch has SAFE_MODE_MAX_FORK_DISTANCE blocks from last common block, alternative branch has 10 blocks more. 
2. Send SAFE_MODE_MAX_FORK_DISTANCE - 1 blocks of main branch and whole alternative branch.
4. Node enter safe mode because alternative branch is more than SAFE_MODE_MIN_POW_DIFFERENCE ahead of main chain and inside SAFE_MODE_MAX_FORK_DISTANCE
5. Send one more block of main chain. 
6. Node exits safe mode because alternative branch is no longer inside SAFE_MODE_MAX_FORK_DISTANCE (it has still more than SAFE_MODE_MIN_POW_DIFFERENCE)
"""
...
class ExitSafeModeForDistantFork(BitcoinTestFramework):
    ...
    def run_test(self):
        with self.run_node_with_connections("Preparation", 0, None, 2) as (conn1, conn2):
            ...
            for _ in range(SAFE_MODE_MAX_FORK_DISTANCE + SAFE_MODE_MIN_POW_DIFFERENCE + 2):
            ...
            send_by_headers(conn1, branch_1_blocks[:SAFE_MODE_MAX_FORK_DISTANCE], do_send_blocks=True)
