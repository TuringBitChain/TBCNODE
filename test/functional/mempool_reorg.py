...
class MempoolCoinbaseTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        spend_101_raw = create_tx(
            self.nodes[0], coinbase_txids[1], node1_address, 4999.99)
        spend_102_raw = create_tx(
            self.nodes[0], coinbase_txids[2], node0_address, 4999.99)
        spend_103_raw = create_tx(
            self.nodes[0], coinbase_txids[3], node0_address, 4999.99)

        # Create a transaction which is time-locked to two blocks in the future
        timelock_tx = self.nodes[0].createrawtransaction(
            [{"txid": coinbase_txids[0], "vout": 0}], {node0_address: 4999.99})
        # Set the time lock
        ...
        spend_102_1_raw = create_tx(
            self.nodes[0], spend_102_id, node1_address, 4999.98)
        spend_103_1_raw = create_tx(
            self.nodes[0], spend_103_id, node1_address, 4999.98)
