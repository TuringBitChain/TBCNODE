...
#This test requires "pip install pyasyncore --break-system-packages"
...
class ZMQNewTopicsTest (BitcoinTestFramework):
    ...
    def check_txnotification(self, seq):
        ...
        self.log.debug(f"Received hashtx2: {bytes_to_hex_str(txhash)}")
        ...
        self.log.debug(f"Received rawtx2 length: {len(body)}")

        # Use node's decoderawtransaction RPC to get the TxID
        # This ensures we use the same TuringTXID algorithm as the node
        try:
            raw_tx_hex = bytes_to_hex_str(body)
            decoded_tx = self.nodes[0].decoderawtransaction(raw_tx_hex)
            computed_txid = decoded_tx['txid']
            
            self.log.debug(f"Node computed TxID: {computed_txid}")
            self.log.debug(f"Expected hash: {bytes_to_hex_str(txhash)}")
            
            # Compare the hex strings directly since both should be the same
            expected_txid = bytes_to_hex_str(txhash)
            
            # Verify that the node's TxID matches the ZMQ hash
            assert_equal(computed_txid, expected_txid)
            self.log.debug("TxID verification PASSED!")
            
        except Exception as e:
            self.log.error(f"TxID verification failed: {str(e)}")
    ...
    def test_activenotifications(self):
        # Skip this test as activezmqnotifications RPC method may not be available
        # in this version of the node
        pass
