...
class AbandonConflictTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [["-mempoolminfeerate=1000"], []]
    ...
    def run_test(self):
        ...
        txC = self.nodes[0].sendtoaddress(
            self.nodes[0].getnewaddress(), Decimal("10"))
        fees = -sum(self.nodes[0].gettransaction(txid)["fee"]
        ...
        assert_equal(balance - newbalance, fees)
        balance = newbalance
        ...
        # Identify the 14.998 TBC output
        nAB = next(i for i, vout in enumerate(self.nodes[0].getrawtransaction(
            txAB1, 1)["vout"]) if vout["value"] == Decimal("14.998"))
        ...
        # Restart the node with a higher mempool minimum fee so the parent tx is no longer in mempool
        # TODO: redo with eviction
        self.stop_node(0)
        self.start_node(0, extra_args=["-mempoolminfeerate=10000"])
        ...
        # Verify that even with a low mempool minimum fee, the tx is not reaccepted from wallet on startup once abandoned
        self.stop_node(0)
        self.start_node(0, extra_args=["-mempoolminfeerate=1000"])
        assert_equal(len(self.nodes[0].getrawmempool()), 0)
