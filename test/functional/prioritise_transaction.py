...
class PrioritiseTransactionTest(BitcoinTestFramework):
    ...
    # Phase 3 (TBC v3.3.0): run_legacy_test removed — used -blockassembler=legacy and
    # -printpriority debug option, both deprecated with LegacyBlockAssembler deletion.
    ...
    def run_test(self):
        # Phase 3 (TBC v3.3.0): LegacyBlockAssembler removed. run_legacy_test no longer
        # runnable (its `-blockassembler=legacy` would now throw at startup, and
        # `-printpriority` debug option was Legacy-only). Only Journaling assembler is
        # supported now.
