...
{
    // TBC uses LogAcceptCategory(BCLog::IPC) instead of
    // util::log::ShouldTraceLog / ShouldDebugLog from upstream.
    // Return Debug if the IPC category is enabled, otherwise Info.
    if (LogAcceptCategory(BCLog::IPC)) return mp::Log::Debug;
    return mp::Log::Info;
}

void IpcLogFn(mp::LogMessage message)
{
    // TBC adaptation: route mp::Log levels via LogPrint(BCLog::IPC, ...) /
    // LogPrintf(...) instead of the upstream LogTrace/LogDebug/LogInfo macros.
...
public:
    ...
    void startLoop(const char* exe_name)
    {
        ...
        m_loop_thread = std::thread([&] {
            // TBC adaptation: util.h pulls in Boost.Signals2 which uses
            // C++17-only allocator traits incompatible with C++20.  Call
            // pthread directly instead of RenameThread("capnp-loop").
            pthread_setname_np(pthread_self(), "capnp-loop");
            mp::LogOptions opts = {
                .log_fn = IpcLogFn,
                .log_level = GetRequestedIPCLogLevel()
            };
