...
void Sock::SendComplete(bsv::span<const unsigned char> data,
    ...
    for (;;) {
        ...
        if (ret > 0) {
            ...
            if (IOErrorIsPermanent(err)) {
                throw std::runtime_error(strprintf("send(): %s", SockNetworkErrorString(err)));
            }
        }
...
void Sock::SendComplete(bsv::span<const char> data,
                        std::chrono::milliseconds timeout,
                        CThreadInterrupt& interrupt) const
{
                                      ...
                                      size_t max_data) const
{
    ...
    for (;;) {
        ...
        case -1: {
            const int err{WSAGetLastError()};
            if (IOErrorIsPermanent(err)) {
                throw std::runtime_error(strprintf("recv(): %s", SockNetworkErrorString(err)));
            }
            break;
        }
        case 0:
...
bool Sock::IsConnected(std::string& errmsg) const
{
    ...
    case -1: {
        const int err = WSAGetLastError();
        if (IOErrorIsPermanent(err)) {
            errmsg = SockNetworkErrorString(err);
            return false;
        }
        return true;
    }
...
#endif
    if (ret) {
        LogPrintf("Error closing socket %d: %s\n", m_socket, SockNetworkErrorString(WSAGetLastError()));
    }
...
std::string SockNetworkErrorString(int err)
{
#if defined(WIN32)
    return Win32ErrorString(err);
#else
    // On BSD sockets implementations, SockNetworkErrorString is the same as SysErrorString.
