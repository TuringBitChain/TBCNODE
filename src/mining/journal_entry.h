...
* Contains a pointer to the transaction itself and fee/sigops accounting details.
*/
class CJournalEntry
{
  public:
    ...
    // Constructors
    CJournalEntry(const CTransactionRef& txn, const Amount& fee, int64_t sigOps)
    : mTxn{txn}, mFee{fee}, mSigOpsCount{sigOps}
    {}

    CJournalEntry(const CTxMemPoolEntry& entry)
    : CJournalEntry{entry.GetSharedTx(), entry.GetFee(), entry.GetSigOpCount()}
    {}
