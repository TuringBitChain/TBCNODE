...
ChaCha20::ChaCha20(bsv::span<const std::byte> key) noexcept 
{
    SetKey(key);
}
...
void ChaCha20::SetKey(bsv::span<const std::byte> key) noexcept {
    assert(key.size() == KEYLEN);
    input[0] = ReadLE32(bsv::UCharCast(key.data() + 0));
    input[1] = ReadLE32(bsv::UCharCast(key.data() + 4));
    input[2] = ReadLE32(bsv::UCharCast(key.data() + 8));
    input[3] = ReadLE32(bsv::UCharCast(key.data() + 12));
    input[4] = ReadLE32(bsv::UCharCast(key.data() + 16));
    input[5] = ReadLE32(bsv::UCharCast(key.data() + 20));
    input[6] = ReadLE32(bsv::UCharCast(key.data() + 24));
    input[7] = ReadLE32(bsv::UCharCast(key.data() + 28));
    input[8] = 0;
    input[9] = 0;
    input[10] = 0;
    input[11] = 0;
...
void ChaCha20::Keystream(bsv::span<std::byte> output) noexcept 
{
    unsigned char* c = bsv::UCharCast(output.data());
...
void ChaCha20::Crypt(bsv::span<const std::byte> in_bytes,
                     bsv::span<std::byte> out_bytes) noexcept 
{
    assert(in_bytes.size() == out_bytes.size());
    const unsigned char* m = bsv::UCharCast(in_bytes.data());
    unsigned char* c = bsv::UCharCast(out_bytes.data());
...
ChaCha20Unrestricted::~ChaCha20Unrestricted()
{
    memory_cleanse(m_buffer.data(), m_buffer.size());
}

void ChaCha20Unrestricted::SetKey(bsv::span<const std::byte> key) noexcept
{
...
void ChaCha20Unrestricted::Crypt(bsv::span<const std::byte> input,
                                 bsv::span<std::byte> output) noexcept 
{
...
void ChaCha20Unrestricted::Keystream(bsv::span<std::byte> out) noexcept 
{
    if (out.empty()) return;
    if (m_bufleft) {
...
FSChaCha20::FSChaCha20(bsv::span<const std::byte> key,
...
void FSChaCha20::Crypt(bsv::span<const std::byte> input, bsv::span<std::byte> output) noexcept
{
    assert(input.size() == output.size());
