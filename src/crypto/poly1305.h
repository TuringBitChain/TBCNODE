...
public:
    ...
    Poly1305(bsv::span<const std::byte> key) noexcept
    {
        assert(key.size() == KEYLEN);
        poly1305_donna::poly1305_init(&m_ctx, bsv::UCharCast(key.data()));
    }

    /** Process message bytes. */
    Poly1305& Update(bsv::span<const std::byte> msg) noexcept
    {
        poly1305_donna::poly1305_update(&m_ctx, bsv::UCharCast(msg.data()), msg.size());
        return *this;
    }

    /** Write authentication tag to 16-byte out. */
    void Finalize(bsv::span<std::byte> out) noexcept
    {
        assert(out.size() == TAGLEN);
        poly1305_donna::poly1305_finish(&m_ctx, bsv::UCharCast(out.data()));
    }
};
