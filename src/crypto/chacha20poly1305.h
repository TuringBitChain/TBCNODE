...
public:
    ...
    AEADChaCha20Poly1305(bsv::span<const std::byte> key) noexcept;

    /** Switch to another 32-byte key. */
    void SetKey(bsv::span<const std::byte> key) noexcept;

    /** 96-bit nonce type. */
    //using Nonce96 = ChaCha20Unrestricted::Nonce96;

    /** Encrypt a message with a specified 96-bit nonce and aad.
     *
     * Requires cipher.size() = plain.size() + EXPANSION.
     */
    void Encrypt(bsv::span<const std::byte> plain, bsv::span<const std::byte> aad, Nonce96 nonce, bsv::span<std::byte> cipher) noexcept
    {
     ...
     */
    void Encrypt(bsv::span<const std::byte> plain1, bsv::span<const std::byte> plain2, bsv::span<const std::byte> aad, Nonce96 nonce, bsv::span<std::byte> cipher) noexcept;

    /** Decrypt a message with a specified 96-bit nonce and aad. Returns true if valid.
     *
     * Requires cipher.size() = plain.size() + EXPANSION.
     */
    bool Decrypt(bsv::span<const std::byte> cipher, bsv::span<const std::byte> aad, Nonce96 nonce, bsv::span<std::byte> plain) noexcept
    {
     ...
     * Requires cipher.size() = plain1.size() + plain2.size() + EXPANSION.
     */
    bool Decrypt(bsv::span<const std::byte> cipher, bsv::span<const std::byte> aad, Nonce96 nonce, bsv::span<std::byte> plain1, bsv::span<std::byte> plain2) noexcept;
     ...
     */
    void Keystream(Nonce96 nonce, bsv::span<std::byte> keystream) noexcept;
};
...
public:
    ...
    FSChaCha20Poly1305(bsv::span<const std::byte> key, uint32_t rekey_interval) noexcept :
     ...
     * Requires cipher.size() = plain.size() + EXPANSION.
     */
    void Encrypt(bsv::span<const std::byte> plain, bsv::span<const std::byte> aad, bsv::span<std::byte> cipher) noexcept
    {
     ...
     * Requires cipher.size() = plain.size() + EXPANSION.
     */
    void Encrypt(bsv::span<const std::byte> plain1, bsv::span<const std::byte> plain2, bsv::span<const std::byte> aad, bsv::span<std::byte> cipher) noexcept;

    /** Decrypt a message with a specified aad. Returns true if valid.
     *
     * Requires cipher.size() = plain.size() + EXPANSION.
     */
    bool Decrypt(bsv::span<const std::byte> cipher, bsv::span<const std::byte> aad, bsv::span<std::byte> plain) noexcept
    {
     ...
     * Requires cipher.size() = plain1.size() + plain2.size() + EXPANSION.
     */
    bool Decrypt(bsv::span<const std::byte> cipher, bsv::span<const std::byte> aad, bsv::span<std::byte> plain1, bsv::span<std::byte> plain2) noexcept;
};
