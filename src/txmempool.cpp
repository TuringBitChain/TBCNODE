...
void CTxMemPool::UpdateTransactionsFromBlock(
    ...
    // Use a set for lookups into vHashesToUpdate.
    std::set<uint256> setAlreadyIncluded(vHashesToUpdate.begin(),
                                         vHashesToUpdate.end());

    // Iterate in reverse, so that whenever we are looking at a transaction we
    // are sure that all in-mempool descendants have already been processed, and
    // setMemPoolChildren is updated before its parents look at it.
    ...
    // During reorg, re-added transactions enter mapTx with their own ancestorsHeight
    // computed at insertion time (based on their mempool parents, which is empty because
    // they were previously confirmed UTXOs). Their existing mempool children had
    // ancestorsHeight=0 when they were added (parents were confirmed, not in mempool).
    // Now those parents are back in the mempool, so children have stale-low heights.
    // Stale-low heights can cause children to sort before their parents in
    // checkJournalAcceptanceNL, breaking CPFP evaluation. Fix by propagating correct
    // heights down to all descendants before the journal check.
    // Note: this does NOT trigger the ancestor limit — that check only applies at new
    // tx admission (CalculateMemPoolAncestors). Existing mempool txs are never evicted
    // solely because their recalculated height exceeds the limit, mirroring the
    // original ancestor-count behavior.
    {
        setEntriesTopoSorted toUpdate;
        for (const txiter& entry : addedTransactions) {
            toUpdate.insert(entry);
        }
        UpdateAncestorsHeightNL(std::move(toUpdate));
    }
...
void CTxMemPool::updateAncestorsOfNL(bool add, txiter it) {
    // add or remove this tx as a child of each of its mempool parents
    for (txiter piter : GetMemPoolParentsNL(it)) {
...
void CTxMemPool::updateForRemoveFromMempoolNL(const setEntries &entriesToRemove,
    ...
    // Sever the child links that point to each removed transaction in the
    // entries for its mempool parents.
                                        ...
                                        const uint256 &hashb)
{
    ...
    // insertionIndex is a topological proxy that never goes stale.
    uint64_t deptha = i->GetInsertionIndex();
    uint64_t depthb = j->GetInsertionIndex();
    if (deptha == depthb) {
...
void CTxMemPool::prioritiseTransactionNL(
    ...
    if (it != mapTx.end()) {
        ...
        // The fee delta changes this transaction's modified fee, which can change
        // journal acceptance for it and its connected transactions.
