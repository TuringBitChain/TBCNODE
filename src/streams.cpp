...
std::size_t AutoFile::detail_fread(bsv::span<std::byte> dst)
{
...
void AutoFile::read(bsv::span<std::byte> dst)
{
...
void AutoFile::write(bsv::span<const std::byte> src)
{
    ...
    if (m_xor.empty()) {
        ...
        while (src.size() > 0) {
            auto buf_now{bsv::span{buf}.first(std::min<size_t>(src.size(), buf.size()))};
