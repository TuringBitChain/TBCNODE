...
CBlockLocator CChain::GetLocator(const CBlockIndex *pindex) const {
    ...
    while (pindex) {
        ...
        // Stop at genesis or at a retained-history root.
        if (pindex->nHeight == 0 || pindex->pprev == nullptr) {
            break;
        }
        // Exponentially larger steps back, plus the genesis block.
        int nHeight = std::max(pindex->nHeight - nStep, 0);
        const CBlockIndex *pindexNext = nullptr;
        if (Contains(pindex)) {
            // Use O(1) CChain index if possible.
            pindexNext = (*this)[nHeight];
        } else {
            // Otherwise, use O(log n) skiplist.
            pindexNext = pindex->GetAncestor(nHeight);
        }
        if (!pindexNext) {
            // The requested height is older than the retained root. Include
            // that root as the final locator entry.
            while (pindex->pprev) {
                pindex = pindex->pprev;
            }
        } else {
