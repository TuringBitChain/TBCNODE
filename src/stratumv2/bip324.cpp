...
BIP324Cipher::BIP324Cipher(const CKey& key, bsv::span<const std::byte> ent32) noexcept :
    m_key(key)
{
...
void BIP324Cipher::Initialize(const EllSwiftPubKey& their_pubkey, bool initiator, bool self_decrypt) noexcept
{
    ...
    const auto& message_header = Params().DiskMagic();
    ...
    CHKDF_HMAC_SHA256_L32 hkdf(bsv::UCharCast(ecdh_secret.data()), ecdh_secret.size(), salt);
    std::array<std::byte, 32> hkdf_32_okm;
    hkdf.Expand32("initiator_L", bsv::UCharCast(hkdf_32_okm.data()));
    (side ? m_send_l_cipher : m_recv_l_cipher).emplace(hkdf_32_okm, REKEY_INTERVAL);
    hkdf.Expand32("initiator_P", bsv::UCharCast(hkdf_32_okm.data()));
    (side ? m_send_p_cipher : m_recv_p_cipher).emplace(hkdf_32_okm, REKEY_INTERVAL);
    hkdf.Expand32("responder_L", bsv::UCharCast(hkdf_32_okm.data()));
    (side ? m_recv_l_cipher : m_send_l_cipher).emplace(hkdf_32_okm, REKEY_INTERVAL);
    hkdf.Expand32("responder_P", bsv::UCharCast(hkdf_32_okm.data()));
    (side ? m_recv_p_cipher : m_send_p_cipher).emplace(hkdf_32_okm, REKEY_INTERVAL);

    // Derive garbage terminators from shared secret.
    hkdf.Expand32("garbage_terminators", bsv::UCharCast(hkdf_32_okm.data()));
    ...
    hkdf.Expand32("session_id", bsv::UCharCast(m_session_id.data()));
...
void BIP324Cipher::Encrypt(bsv::span<const std::byte> contents, bsv::span<const std::byte> aad, bool ignore, bsv::span<std::byte> output) noexcept
{
...
uint32_t BIP324Cipher::DecryptLength(bsv::span<const std::byte> input) noexcept
{
...
bool BIP324Cipher::Decrypt(bsv::span<const std::byte> input, bsv::span<const std::byte> aad, bool& ignore, bsv::span<std::byte> contents) noexcept
{
