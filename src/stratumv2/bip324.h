...
public:
    ...
    BIP324Cipher(const CKey& key, bsv::span<const std::byte> ent32) noexcept;
     ...
     */
    void Encrypt(bsv::span<const std::byte> contents, bsv::span<const std::byte> aad, bool ignore, bsv::span<std::byte> output) noexcept;

    /** Decrypt the length of a packet. Only after Initialize().
     *
     * It must hold that input.size() == LENGTH_LEN.
     */
    unsigned DecryptLength(bsv::span<const std::byte> input) noexcept;
     ...
     */
    bool Decrypt(bsv::span<const std::byte> input, bsv::span<const std::byte> aad, bool& ignore, bsv::span<std::byte> contents) noexcept;

    /** Get the Session ID. Only after Initialize(). */
    bsv::span<const std::byte> GetSessionID() const noexcept { return m_session_id; }

    /** Get the Garbage Terminator to send. Only after Initialize(). */
    bsv::span<const std::byte> GetSendGarbageTerminator() const noexcept { return m_send_garbage_terminator; }

    /** Get the expected Garbage Terminator to receive. Only after Initialize(). */
    bsv::span<const std::byte> GetReceiveGarbageTerminator() const noexcept { return m_recv_garbage_terminator; }
};
