...
void Sv2Transport::StartSendingHandshake() noexcept
{
    ...
    m_cipher.GetHandshakeState().WriteMsgEphemeralPK(bsv::MakeWritableByteSpan(m_send_buffer));
...
void Sv2Transport::SendHandshakeReply() noexcept
{
    ...
    m_cipher.GetHandshakeState().WriteMsgES(bsv::MakeWritableByteSpan(m_send_buffer));
...
Sv2Transport::Sv2BytesToSend Sv2Transport::GetBytesToSendSv2(bool have_next_message) const noexcept
{
    ...
    return {
        bsv::span{m_send_buffer}.subspan(m_send_pos),
...
bool Sv2Transport::SetMessageToSend(Sv2NetMsg& msg) noexcept
{
    ...
    // Payload
    bsv::span<std::byte> payload_plain = bsv::MakeWritableByteSpan(msg.m_msg);
    // TODO: truncate very long messages, about 100 bytes at the start and end
    //       is probably enough for most debugging.
    // ////LogPrintLevel(BCLog::SV2, BCLog::Level::Trace, "Payload: %s\n", HexStr(payload_plain));
    bsv::span<std::byte> payload_encrypted{buffer_span.subspan(SV2_HEADER_ENCRYPTED_SIZE, encrypted_msg_size)};
...
bool Sv2Transport::ReceivedBytes(bsv::span<const uint8_t>& msg_bytes) noexcept
{
    AssertLockNotHeld(m_send_mutex);
    AssertLockNotHeld(m_recv_mutex);
    ...
    while (!msg_bytes.empty()) {
        ...
        // Copy data to buffer.
        m_recv_buffer.insert(m_recv_buffer.end(), bsv::UCharCast(msg_bytes.data()), bsv::UCharCast(msg_bytes.data() + max_read));
...
bool Sv2Transport::ProcessReceivedEphemeralKeyBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == Sv2HandshakeState::ELLSWIFT_PUB_KEY_SIZE) {
        ...
        // TODO handle failure
        // TODO: MakeByteSpan instead of bsv::MakeWritableByteSpan
        m_cipher.GetHandshakeState().ReadMsgEphemeralPK(bsv::MakeWritableByteSpan(m_recv_buffer));
        m_recv_buffer.clear();
...
bool Sv2Transport::ProcessReceivedHandshakeReplyBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == Sv2HandshakeState::HANDSHAKE_STEP2_SIZE) {
        // TODO handle failure
        // TODO: MakeByteSpan instead of bsv::MakeWritableByteSpan
        bool res = m_cipher.GetHandshakeState().ReadMsgES(bsv::MakeWritableByteSpan(m_recv_buffer));
        if (!res) return false;
        m_recv_buffer.clear();
        m_cipher.FinishHandshake();
        SetReceiveState(RecvState::APP);
...
bool Sv2Transport::ProcessReceivedPacketBytes() noexcept
{
    ...
    if (m_recv_buffer.size() == SV2_HEADER_ENCRYPTED_SIZE) {
        ...
        // auto tmpDate = ss_header.data();
        // for(int i = 0; i < ss_header.size(); ++i)
        // {
        //     LogPrintf("ss_header:%d ",static_cast<int>(tmpDate[i]));
        // }
        // LogPrintf("ss_header over\n");
...
Sv2NetMsg& Sv2Transport::GetReceivedMessage(std::chrono::microseconds time, bool& reject_message) noexcept
{
    AssertLockNotHeld(m_recv_mutex);
    LOCKMt(m_recv_mutex);
