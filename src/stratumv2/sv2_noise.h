...
private:
    ...
    uint256 GetHash();
    void SignSchnorr(const CKey& authority_key, bsv::span<unsigned char> sig);
...
public:
     ...
     */
    [[nodiscard]] bool DecryptWithAd(bsv::span<const std::byte> associated_data, bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain);
    /** Encrypt message
     * @param[in] associated_data associated data
     ...
     * @returns whether decryption succeeded
     */
    [[nodiscard]] bool EncryptWithAd(bsv::span<const std::byte> associated_data, bsv::span<const std::byte> plain, bsv::span<std::byte> ciphertext);
     ...
     */
    [[nodiscard]] bool EncryptMessage(bsv::span<const std::byte> plain, bsv::span<std::byte> ciphertext);
     ...
     */
    [[nodiscard]] bool DecryptMessage(bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain);
...
public:
    ...
    std::vector<uint8_t> PROTOCOL_NAME_DOUBLE_HASH{146, 47, 163, 46, 79, 72, 124, 13, 89, 202, 163, 190, 215, 137, 156, 227,
    ...
    void MixHash(const bsv::span<const std::byte> input);
    void MixKey(const bsv::span<const std::byte> input_key_material);
    [[nodiscard]] bool EncryptAndHash(bsv::span<const std::byte> plain, bsv::span<std::byte> ciphertext);
    [[nodiscard]] bool DecryptAndHash(bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain);
...
private:
    ...
    void HKDF2(const bsv::span<const std::byte> input_key_material, uint8_t out0[Sv2CipherState::HASHLEN], uint8_t out1[Sv2CipherState::HASHLEN]);
};
...
public:
    ...
    {
        m_our_static_ellswift_pk = static_key.EllSwiftCreate(bsv::MakeByteSpan(GetRandHash()));
    };
...
public:
    ...
    bool DecryptMessage(bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain);
    /** Encrypts a message. May only be called after FinishHandshake() */
    [[nodiscard]] bool EncryptMessage(bsv::span<const std::byte> input, bsv::span<std::byte> output);
