...
{
public:
    using Sv2NetHeader  = node::Sv2NetHeader;
...
private:
    ...
    static std::optional<std::string> GetMessageType(bsv::span<const uint8_t>& contents) noexcept;
...
public:
    ...
    bool ReceivedBytes(bsv::span<const uint8_t>& msg_bytes) noexcept override EXCLUSIVE_LOCKS_REQUIRED(!m_recv_mutex, !m_send_mutex);

    // Override not implemented
    Sv2NetMsg& GetReceivedMessage(std::chrono::microseconds time, bool& reject_message) noexcept override EXCLUSIVE_LOCKS_REQUIRED(!m_recv_mutex);
