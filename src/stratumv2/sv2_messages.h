...
struct Sv2NewTemplateMsg
{
    ...
    void Serialize(Stream& s) const
    {
        ...
        if (m_coinbase_tx_outputs_count > 0) {
            ...
            s.write(bsv::MakeByteSpan(outputs_bytes));
        } else {
...
struct Sv2RequestTransactionDataSuccessMsg
{
    ...
    void Serialize(Stream& s) const
    {
        ...
        //     s.write(bsv::MakeByteSpan(m_excess_data));
        // }
        s << static_cast<uint16_t>(0);
        ...
        for (const auto& tx : m_transactions_list) {
            ...
            s << tx_byte_len;
            s.write(bsv::MakeByteSpan(ss_tx));
        }
    };
};
...
public:
    ...
    // Sv2NetMsg(SerializedNetMsg&& net_msg)
    // {
    //     Assume(net_msg.m_type == "");
    //     DataStream ss(bsv::MakeByteSpan(net_msg.data));
    //     Unserialize(ss);
    // };

    // TMP;
    // Unwrap CNetMsg
    Sv2NetMsg(NetMessage net_msg)
    {
    ...
    {
        s << m_sv2_header;
        s.write(bsv::MakeByteSpan(m_msg));
    }
    ...
    void Unserialize(Stream& s)
    {
        uint8_t msg_type;
        s >> msg_type;
        m_sv2_header.m_msg_type = static_cast<Sv2MsgType>(msg_type);
        m_msg.resize(s.size());
        s.read(bsv::MakeWritableByteSpan(m_msg));
    }
};
