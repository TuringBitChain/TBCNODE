...
void Sv2SignatureNoiseMessage::SignSchnorr(const CKey& authority_key, bsv::span<unsigned char> sig)
{
...
bool Sv2CipherState::DecryptWithAd(bsv::span<const std::byte> associated_data, bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain)
{
    ...
    Nonce96 nonce = {0, m_nonce};
    auto key = bsv::MakeByteSpan(m_key);
...
bool Sv2CipherState::EncryptWithAd(bsv::span<const std::byte> associated_data, bsv::span<const std::byte> plain, bsv::span<std::byte> ciphertext)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());
...
bool Sv2CipherState::EncryptMessage(bsv::span<const std::byte> plain, bsv::span<std::byte> ciphertext)
{
    ...
    for (size_t i = 0; i < num_chunks; ++i) {
        ...
        bsv::span<std::byte> chunk = ciphertext.subspan(bytes_written, chunk_size);
        bsv::span<std::byte> chunk_plain = ciphertext.subspan(bytes_written, chunk_size - Poly1305::TAGLEN);
...
bool Sv2CipherState::DecryptMessage(bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());
    ...
    while (processed < ciphertext.size()) {
        ...
        bsv::span<std::byte> chunk_cipher = ciphertext.subspan(processed, chunk_size);
        bsv::span<std::byte> chunk_plain = plain.subspan(plain_position, chunk_size - Poly1305::TAGLEN);
...
void Sv2SymmetricState::MixHash(const bsv::span<const std::byte> input)
{
    m_hash_output = (HashWriter{} << m_hash_output << input).GetSHA256();
}

void Sv2SymmetricState::MixKey(const bsv::span<const std::byte> input_key_material)
{
...
{
    return HexStr(bsv::MakeByteSpan(m_chaining_key));
}
...
void Sv2SymmetricState::HKDF2(const bsv::span<const std::byte> input_key_material, uint8_t out0[Sv2CipherState::HASHLEN], uint8_t out1[Sv2CipherState::HASHLEN])
{
    uint8_t tmp_key[Sv2CipherState::HASHLEN];
    CHMAC_SHA256 tmp_mac(m_chaining_key, Sv2CipherState::HASHLEN);
    tmp_mac.Write(bsv::UCharCast(input_key_material.data()), input_key_material.size());
...
bool Sv2SymmetricState::EncryptAndHash(bsv::span<const std::byte> plain, bsv::span<std::byte> ciphertext)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());

    if (!m_cipher_state.EncryptWithAd(bsv::MakeByteSpan(m_hash_output), plain, ciphertext)) {
        return false;
    }
    MixHash(ciphertext);
    return true;
...
bool Sv2SymmetricState::DecryptAndHash(bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain)
{
    Assume(Sv2Cipher::EncryptedMessageSize(plain.size()) == ciphertext.size());
    ...
    bool res = m_cipher_state.DecryptWithAd(bsv::MakeByteSpan(m_hash_output), ciphertext, plain);
    if (!res) return false;
...
{
    m_ephemeral_key = key;
    m_our_ephemeral_ellswift_pk = m_ephemeral_key.EllSwiftCreate(bsv::MakeByteSpan(GetRandHash()));
};
...
void Sv2HandshakeState::GenerateEphemeralKey() noexcept
{
    ...
    CKey key;
    key.MakeNewKey(true);
    SetEphemeralKey(std::move(key));
};

void Sv2HandshakeState::WriteMsgEphemeralPK(bsv::span<std::byte> msg)
{
...
void Sv2HandshakeState::ReadMsgEphemeralPK(bsv::span<std::byte> msg)
{
...
void Sv2HandshakeState::WriteMsgES(bsv::span<std::byte> msg)
{
...
bool Sv2HandshakeState::ReadMsgES(bsv::span<std::byte> msg)
{
    ...
    if (!cert.Validate(m_authority_pubkey.value())) {
        ...
        LogPrintf("Invalid certificate: %s\n", HexStr(remote_cert_bytes));
        return false;
    }
...
bool Sv2Cipher::DecryptMessage(bsv::span<std::byte> ciphertext, bsv::span<std::byte> plain)
{
...
bool Sv2Cipher::EncryptMessage(bsv::span<const std::byte> input, bsv::span<std::byte> output)
{
