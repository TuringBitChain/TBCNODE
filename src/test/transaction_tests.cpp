...
BOOST_AUTO_TEST_CASE(test_IsStandard) {
    ...
    // The dust threshold is fixed and does not depend on the relay fee.
    const bool isGenesisEnabled = IsGenesisEnabled(testConfig, 1);
    const Amount nDustThreshold =
        t.vout[0].GetDustThreshold(dustRelayFee, isGenesisEnabled);
    ...
    // Changing the relay fee does not change the fixed dust threshold.
